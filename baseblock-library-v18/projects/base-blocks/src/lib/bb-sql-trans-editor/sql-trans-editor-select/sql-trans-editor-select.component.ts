import { Input, Component, HostListener, ViewChild , ViewChildren, ElementRef , QueryList , Output, EventEmitter, TemplateRef, ViewContainerRef, SimpleChanges } from '@angular/core';
...
import { BBTransTreeviewComponent } from '../../bb-trans-treeview';
...
@Component( {
    selector: 'sql-trans-editor-select',
    templateUrl: './sql-trans-editor-select.component.html',
    styleUrls: ['./sql-trans-editor-select.component.css'],
    providers: [TitleCasePipe],
})

export class SqlTransEditorSelectComponent {
   ...
   //Changes by shrutika on 29-11-21 [Start] for No data found occur when multiple time click on preview after changing only  where condition
    ...
    @Input() isTransModel: boolean = false;
   ...
   currentGroupBoxName = "";
    ...
    @ViewChild('treeViewTemp') treeViewTemp: BBTransTreeviewComponent | any;
    ...
    constructor(private matIconRegistry : MatIconRegistry, private domSanitizer : DomSanitizer, private sqlService : SQLTransEditorService, 
    ...
    ngOnInit()
    {
        ...
        console.log("line no 387 oldSlectedTableDataArray", this.oldSlectedTableDataArray)
        console.log("line no 388 finalTableArray", this.finalTableArray);
        console.log("line no 389 joinsArray", this.joinsArray);
   	...
   	//Change by shrutika on 05-10-21 for schema designer group box 
        ...
        {
            this.SqlModelData = JSON.parse(JSON.stringify(this.SqlModelDataObj));
            console.log("line no 418 this.SqlModelData", this.SqlModelData);
        }
        ...
        //Change by shrutika on 08-03-21 [Start] for draw sql editor according to ba type.
            ...
            {
                // console.log("line no 436 this.SqlModelData['VISUAL_NAME']", this.SqlModelData['VISUAL_NAME']);
                // console.log("line no 437 this.SqlModelData['SCHEMA_NAME']", this.SqlModelData['SCHEMA_NAME']);
                // if( this.SqlModelData['VISUAL_NAME'] != undefined && this.SqlModelData['SCHEMA_NAME'] != undefined )
                console.log("line no 439 this.schemaList", this.schemaList);
                {
                    for(let i=0; i<this.schemaList.length; i++)
                    {
                        let schema = this.schemaList[i];
                        console.log("line no 441 schema", schema);
                        console.log("line no 442 this.SqlModelData['SCHEMA_NAME']", this.SqlModelData['SCHEMA_NAME']);
                        if(schema['schemaName'] == this.SqlModelData['SCHEMA_NAME'])
                        {
                            ...
                            console.log("line no 445 this.defaultVisual", this.defaultVisual);
                            currentSchemalist.push(schema);
                        }
                    }
                    this.schemaList = currentSchemalist;
                    console.log("line no 454 this.schemaList", this.schemaList);
                    this.currentSchema = this.schemaList[0];
                    console.log("line no 456 this.currentSchema", this.currentSchema);
                }
            }
            ...
            if(this.editFlag != 'A' && this.isSqlView == true)
            {
                this.schemaEditorSelector(3);
            }
            else if(this.editFlag != 'A' && (this.isSqlView == false || this.isSqlView == undefined))
            {
                ...
                // if(this.SqlModelData != undefined && this.SqlModelData['SQLModel'] != undefined && this.SqlModelData['SQLModel']['COLUMNS'] != undefined && this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'][0]['column'] != undefined)
                if(this.SqlModelData != undefined && this.SqlModelData['transaction'] != undefined && this.SqlModelData['transaction']['sql_models'] != undefined && this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'] != undefined)
                {
                    console.log("line no 475:::::::: SqlModelData", this.SqlModelData);
                    let currentColumnArray = this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'][0]['column']
                    for(let i=0; i<currentColumnArray.length; i++)
                    {
                        let currentGrpName = currentColumnArray[i]['groupName'];
                        console.log("line no 486 currentGrpName", currentGrpName);
        ...
        if (this.SqlModelData !== undefined && this.SqlModelData['VISUAL_NAME'] !== undefined) {
            if (this.editorVisuals) {
                for (let i = 0; i < this.editorVisuals.Visuals.length; i++) {
                    ...
                    if (
                        this.SqlModelData['transaction'] !== undefined &&
                        this.SqlModelData['transaction']['sql_models'] !== undefined &&
                        this.SqlModelData['transaction']['sql_models'][0] !== undefined &&
                        this.SqlModelData['transaction']['sql_models'][0]['sql_model'] !== undefined &&
                        this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0] !== undefined &&
                        this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0][this.param1] !== undefined
                    ) {
                        let sqlArray = this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'][0]['column'];
    
                        for (let j = 0; j < sqlArray.length; j++) {
                            for (let k = 0; k < visual.ColumnGroups.length; k++) {
                                if (sqlArray[j]['group_name'] === visual.ColumnGroups[k]['GroupName']) {
    ...
    changeVisualOnChangeSchema(){
        ...
        console.log("line no 597  this.editorVisuals",  this.editorVisuals)
        if(this.editorVisuals)
        {
    ...
    getColType(col_type: any)
    {
        let colTypeNew = 'string';

        if(col_type == 'CHAR' || col_type == 'VARCHAR2')
        {
            colTypeNew = 'string';
        }
	...
	//Added by shrutika on 27-09-21 for schema designer [End]
        ...
        else
        {
            ...
            if (document.getElementsByClassName('leftQuickFilterSidePanel') != null)
            {
                const leftSidePanel = document.getElementById('gproLeftDesignSidePanl').classList.remove("leftQuickFilterSidePanel");
                console.log("line no 1384 sidePanelElements", leftSidePanel);
            }
        }
    ...
    onNext()
    {
        ...
        if(this.currentSchema != undefined && this.currentSchema['baType'] != 'F' &&  this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'][0]['column'].length <= 0)
         { 
    ...
    getChangeData(changedData: any)
    {
        ...
        let currColData = selectedData['columnData'];
        var neWColumData: any = {};
        var calColVal : boolean = false;
        // Added by Samruddhi for Freehandsql
        let currColumnVal : boolean = false;
        if(currColData.checked)
        {
            ...
            if(this.SqlModelData != undefined && 
                ...
                this.SqlModelData['transaction']['sql_models'][0] != undefined && 
                this.SqlModelData['transaction']['sql_models'][0]['sql_model'] != undefined &&  
                this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'] != undefined) 
            {
                let columnJsonObj = {}
                let sqlModelObj = this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'];
                let currColObj;
              /*  for (let i = 0; i < sqlModelObj.length; i++) 
                {
                    let columnItem = sqlModelObj[i]['column'];
                    if (columnItem && columnItem['name'] === currColData['name']) 
                    {
                        columnJsonObj['column'] = columnItem; 
                        currColObj = columnItem;  
                        currColumnVal = true;
                        calColVal = true;
                    }
                }
                if (currColumnVal && currColObj != undefined) 
		{
                    // Added by Samruddhi for Freehandsql
                    console.log("line no 1069 currColumnVal::::::::",currColObj);
                    currColObj['db_name'] = currColData['db_name'];
                    currColObj['name'] = currColData['name'];
                    currColObj['db_size'] = currColData['db_size'];
                    currColObj['key'] = currColData['key'];
                    currColObj['col_id'] = currColData['col_id'];
                    currColObj['col_type'] = currColData['col_type'];
                    currColObj['default_function'] = currColData['default_function'];
                    currColObj['hidden'] = currColData['hidden'];
                    currColObj['format'] = currColData['format'];
                    currColObj['descr'] = currColData['descr'];
                    currColObj['expression'] = currColData['expression'];
                    currColObj['checked'] = true;
                    currColObj['table_name'] = currColData['table_name'];
                    currColObj['table_display_name'] = currColData['table_display_name'];
                    currColObj['group_name'] = currColData['group_name'];
                    currColObj['heading'] = currColData['heading'];
                    currColObj['default_value'] = currColData['default_value'];
                    currColObj['edit_mask'] = currColData['edit_mask'];
                    currColObj['display'] = currColData['display'];
                    currColObj['lookup'] = currColData['lookup'];
                    currColObj['mandatory'] = currColData['mandatory'];
                    currColObj['protect'] = currColData['protect'];
                    currColObj['x'] = currColData['x'];
                    currColObj['y'] = currColData['y'];
                    currColObj['tabsequence'] = currColData['tabsequence'];
                    currColObj['item_change'] = currColData['item_change'];
                    currColObj['validations'] = currColData['validations'];
                    console.log('1064 currColObj',currColObj)
                } 
                else */
		        {    
                    console.log("line no 1047 else ::::::::::::::  ", this.SqlModelData)  
                    if (this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'] !== undefined) 
                    {
                        let isColumnExits = false;
                        for (let i = 0; i < sqlModelObj.length; i++) 
                        {
                            let columnObj = sqlModelObj[i]['column'];
                            {
                                if (columnObj !== undefined) 
                                {
                                    if (columnObj['db_name'] === currColData['db_name'] && columnObj['name'] === currColData['name']) 
                                    {
                                        isColumnExits = true;
                                        break;
                                    }
                        ...
                        {
                            let columnJsonObj = {};
                            columnJsonObj['column'] = currColData;
                            this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'].push(columnJsonObj);
                        }
                    }
                }
        ...
        {
            if(this.SqlModelData != undefined && this.SqlModelData['transaction'] != undefined && this.SqlModelData['transaction']['sql_models'][0] != undefined && this.SqlModelData['transaction']['sql_models'][0]['sql_model'] != undefined && this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'] != undefined)
            {
                let sqlArray = this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'];
                for (let i = 0; i < sqlArray.length; i++) 
                {
                    let column = sqlArray[i]['column'];
                    if (column && column['db_name'] === currColData['db_name'] && column['table_name'] == currColData['table_name']) 
                    {
                        console.log("line no 1116::::::::::")
                        column['checked'] = false;
                        break;
                    }
                }
                this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'] = sqlArray.filter((item: any) => item['column']['checked']);
            }
        }
        neWColumData['column'] = selectedData;
        console.log("line no 1131 neWColumData", neWColumData);
        var newColumnArray: any = [];
        newColumnArray.push(neWColumData);
        console.log("line no 1134 newColumnArray", newColumnArray);
        selectedData = newColumnArray;
        console.log("line no 1136 selectedData", selectedData);
        var cuurentData: any = {};
        cuurentData['column'] = selectedData;
        console.log("line no 1139 cuurentData", cuurentData);

        const columnData = cuurentData['column'][0]['column']['columnData'];

        currColData['lookup'] = currColData['lookup'] === "" ? columnData['lookup'] : JSON.stringify(columnData['lookup'], null, 2);
        currColData['edit_mask'] = currColData['edit_mask'] === "" ? columnData['edit_mask'] : JSON.stringify(columnData['edit_mask'], null, 2);
        currColData['item_change'] = currColData['item_change'] === "" ? columnData['item_change'] : JSON.stringify(columnData['item_change'], null, 2)
        currColData['validations'] = currColData['validations'] === "" ? columnData['validations'] : JSON.stringify(columnData['validations'], null, 2);
	    
        if(this.editorVisuals != undefined)
        {
            currColData['group_name'] = this.defaultGrpName;
            if(currColData['checked'] == true)
            {
                if(this.editorVisuals)
                {
                    visualLoop:
                    for(let i=0; i<this.editorVisuals.Visuals.length; i++)
                    {
                        ...
                        if(this.defaultVisual == visual['VisualName'])
                        {
                            for(let j=0; j<visual.ColumnGroups.length; j++)
                            {
                                if(this.defaultGrpName == visual.ColumnGroups[j]['GroupName'] && this.SqlModelData != undefined && this.SqlModelData['transaction'] != undefined && this.SqlModelData['transaction']['sql_models'] != undefined && this.SqlModelData['transaction']['sql_models'][0] != undefined && this.SqlModelData['transaction']['sql_models'][0]['sql_model'] != undefined && this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'] != undefined)
                                    {
                                        currColData['group_name'] = visual.ColumnGroups[j]['StandardName'];
                                        let sqlArray = this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'];                                  
                                        for (let i = 0; i < sqlArray.length; i++) 
                                        {
                                            let columnObj = sqlArray[i]['column'];
                                            if (columnObj && (columnObj['db_name'] === currColData['db_name'] && columnObj['table_name'] == currColData['table_name']) || (columnObj['db_name'] == currColData['db_name'] && columnObj['name'] == currColData['name']))
                                            {
                                                columnObj['StandardName'] = currColData['StandardName'];
                                                break;
                                            }
                                        }
                                        if((calColVal != undefined && !calColVal) || (currColumnVal != undefined && !currColumnVal))
                                        {
                                            visual.ColumnGroups[j]['COLUMNS'].push(currColData);
                                        }
                                        break visualLoop;
                                }
                            }
            ...
            else
            {
                ...
                {
                   let visual = this.editorVisuals['Visuals'][i];
                   console.log("line no 117 visual", visual);
                    if(this.defaultVisual == visual['VisualName'])
                    {
                        for(let j=0; j<visual.ColumnGroups.length; j++)
                        {
                            ...
                            var dbName = currColData.db_name;
                            var tblNAme = currColData.table_name;
                            let calSeq = currColData.calc_seq;
                            let calExp = currColData.expression;
                            if (calSeq != undefined) {
                            ...
                            if( index != -1 )
                            {
                                ...
                                console.log("line no 1205 his.currentVisual.ColumnGroups[j]['COLUMNS']", this.currentVisual.ColumnGroups[j]['COLUMNS']);
                                break breakVisualLoop;
                            }
                        }
                    }
                }
                console.log("line no 1238 :::::::::::::")
    ...
    {
        console.log("line no 1248 buildConfigForCriteria::::::::::called")
        try
        {
            var tables: any[] = [];
            var jsonData: any = {
            ...
            if(typeof parsed === 'object' && parsed !== null && !Array.isArray(parsed))
            {
                ...
                {
                    let column = {} = tables[i];
                    let fieldName = column['table_name'] + '.' + column['db_name'];
                    ...
                    let fieldDisplayName = column['table_name'] + '.' + column['name'];
    ...
    {
        console.log("line no 1297 updateTablesArrayOnValidate called:::::::::")
        ...
        {
            console.log("line no 1302 tablesArray", this.tablesArray);
	        for(let i=0; i<this.tablesArray.length; i++)
	        {
	            let columns = this.tablesArray[i]['COLUMN'];
                    console.log("line no 1306 columns", columns);
	            if( column['table_name'] == this.tablesArray[i]['TABLE_NAME'])
	            {
	                for(var j=0; j<columns.length; j++)
	                {
	                    ...
	                    if(column['db_name'] == col['db_name'] && column['table_name'] == col['table_name'])
	                    {
	                        if(column['checked'])
	                        {
	                            col['checked'] = true;
		...
		else
		{
			for(let i=0; i<this.finalTableArray.length; i++)
	        {
                ...
                console.log("line no 1374 currentColArr", currentColArr);
	            for(let j = 0; j < currentColArr.length; j++)
                {
                    if( column['db_name'] == currentColArr[j]['db_name'])
                    {
                        if(column['checked'])
                        {
    ...
    deleteColumn( column : any)
    {
        ...
        if(this.SqlModelData != undefined && this.SqlModelData['transaction'] != undefined && this.SqlModelData['transaction']['sql_models'] && this.SqlModelData['transaction']['sql_models'][0] != undefined && this.SqlModelData['transaction']['sql_models'][0]['sql_model'] != undefined && this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'] != undefined)
        {
            let sqlArray = this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'];
            sqlLoop:
            for(let a=0; a<sqlArray.length; a++)
            {
                ...
                if(column['db_name'] == columnNew['db_name'] && column['table_name'] == columnNew['table_name'])
                {
    ...
    updateGroupsData()
    {
        console.log("line no 1439 updateGroupsData called :::::::::")
        ...
        if(this.SqlModelData != undefined && this.SqlModelData['transaction'] != undefined && this.SqlModelData['transaction']['sql_models'] != undefined && this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'] != undefined)
        {
            let sqlArray = this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'];
            console.log("line no 1455 sqlArray", sqlArray);
            this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns']= sqlArray.filter((item: any) => item['column']['checked']);
        }
    }

    getImgSrc(col_type: any)
    {
        // console.log("line no 1439", col_type)
        var imgUrl;
        if(col_type == "CHAR" || col_type == "VARCHAR2")
        {
    ...
    isExpanded(columnExpansionPanel: MatExpansionPanel, col_type: any)
    {
       if(columnExpansionPanel.expanded)
        {
            ...
            if(col_type == "CHAR" || col_type == "VARCHAR2" || col_type == "VARCHAR" || col_type == "CHAR2" || col_type == "STRING")
            {
            ...
            else if(col_type == "DATE" || col_type == "DATETIME") 
            {
   ...
   isExpandedwindows(columnExpansionPanel: MatExpansionPanel, col_type: any)
   {
      if(columnExpansionPanel.expanded)
       {
          if(col_type == "NUMBER") 
   ...
   //Added by Pranjali on 26-09-2023 for Window Function[End]
    ...
    {
        columnData['FUNCTION'] = '';
        this.isExpanded(columnExpansionPanel, columnData['col_type']);
    ...
    setExpression(columnData: any)
    {
        ...
        let val = column['default_function'];
		let expr;
            if(val != undefined && val != '' && val != null && column['table_name'] == undefined)
            {
			    expr = column['db_name'];
            ...
            if(val != undefined && val != '' && val != null && column['table_name'] != undefined)
            {
			    expr = column['table_name'] + '.' + column['db_name'];
    ...
    onDropSelect(event: CdkDragDrop<any[]> | any, index?: any) {
        ...
        if (event.previousContainer === event.container) {
            ...
            if (event.previousContainer.id === 'schemaList' || event.previousContainer.id === 'columnList') 
            {
                ...
                if (this.currentVisual) {
                    ...
                    if (this.SqlModelData !== undefined  && this.SqlModelData['transaction'] !== undefined && this.SqlModelData['transaction']['sql_models'] !== undefined && this.SqlModelData['transaction']['sql_models'][0] !== undefined && this.SqlModelData['transaction']['sql_models'][0]['sql_model'] !== undefined && this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'] !== undefined) 
                    {
                        // this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'].push(droppedCol);
                        let columnJsonObj = {};
                        columnJsonObj['column'] = droppedCol;
                        this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'].push(columnJsonObj);
                        console.log("line no 1656 this.SqlModelData", this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns']);
                    }
                    // Changed by Samruddhi on 17-05-2022 when we drag and drop a columns in groupbox the column name get disappeared
                    console.log("line no 1664 finalTableArray", this.finalTableArray);
                    ...
                    if (this.currentCalArray != undefined) {
                        for (let i = 0; i < this.currentCalArray.length; i++) {
                            if (this.currentCalArray[i] != undefined && this.currentCalArray[i]['columnData']['name'] == droppedCol['col_name']) {
                                ...
                                if (this.SqlModelData != undefined && this.SqlModelData['transaction'] != undefined && this.SqlModelData['transaction']['sql_models'] != undefined && this.SqlModelData['transaction']['sql_models'][0] != undefined && this.SqlModelData['transaction']['sql_models'][0]['sql_model'] != undefined && this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'] != undefined) 
                                {
                                    this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'].push(this.currentCalArray[i]['columnData']);
                                }
        ...
        if (this.SqlModelData != undefined && this.SqlModelData['transaction'] != undefined && this.SqlModelData['transaction']['sql_models'] != undefined && this.SqlModelData['transaction']['sql_models'][0] != undefined && this.SqlModelData['transaction']['sql_models'][0]['sql_model'] != undefined && this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'] != undefined) 
        {
            arrOfColumns = this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'];
        }
        // Changed by Samruddhi for Freehandsql
        // if( this.listData.baType != 'F' )
        if (this.listData != undefined && this.listData.baType != 'F') {
        ...
        console.log("line no 1724 this.currentVisual", this.currentVisual);
                ...
                for (let i = 0; i < this.currentVisual.ColumnGroups.length; i++) {
                    for (let j = 0; j < this.currentVisual.ColumnGroups[i]['COLUMNS'].length; j++) {
                        if (droppedCol.col_name == this.currentVisual.ColumnGroups[i]['COLUMNS'][j]['col_name'] && droppedCol.calc_seq == this.currentVisual.ColumnGroups[i]['COLUMNS'][j]['calc_seq'] && event.previousContainer.id === 'CalCulationContentID') {
                            ...
                            // this.currentVisual.ColumnGroups[i]['COLUMNS'][j]['col_type'] = "NUMBER";
                            if (droppedCol['col_datatype'] == 'N') {
                                ...
                                this.currentVisual.ColumnGroups[i]['COLUMNS'][j]['col_type'] = "DATE";
                                ...
                                this.currentVisual.ColumnGroups[i]['COLUMNS'][j]['col_type'] = "CHAR";
                                this.currentVisual.ColumnGroups[i]['COLUMNS'][j]['DEFAULTFUNCTION'] = "";
                                this.currentVisual.ColumnGroups[i]['COLUMNS'][j]['FUNCTION'] = "";
            ...
            if (groups.length > 0) {
                groups.forEach((group: any) => {
                    if (group.GroupName == this.defaultGrpName) {
                        ...
                        for (let i = 0; i < arrOfColumns.length; i++) {
                            if (arrOfColumns[i].db_name == droppedCol.db_name && arrOfColumns[i].table_name == droppedCol.table_name) {
      ...
      for(let i=0; i<this.currentVisual.ColumnGroups.length; i++)
        {
            ...
            {
                if(!allowedType.includes(column['col_type']))
                {
  ...
  deleteColFromSqlArray(column: any)
  {
    ...
    let tblNAme = column.table_name;
    let filteredObj = this.sqlColumArray.find(function(item, i)
    {
        if(item.db_name === dbName && item.table_name === tblNAme)
        {
            index = i;
            return i;
        }
  ...
  dragEls!:QueryList<CdkDrag>;
    ...
    buildTablesArray()
    {
        console.log("line no 2032 schemaDataNew", this.schemaDataNew);
                    ...
                    for (var tableKey of Object.keys(this.tablesArray[i])) 
                    {
                        ...
                        {                            
                            ...
                            if( isArray )
                            {
                                ...
                                {
                                    var cuurentColumneData = {} =this.tablesArray[i][tableKey][j];
                                    ...
                                    {
                                        this.tablesArray[i][tableKey][j]["type"] = this.getColType(cuurentColumneData['col_type']);      
                                    }
                                    ...
                                    if (cuurentColumneData['table_name'] != undefined && cuurentColumneData['table_name'] != '') 
                                    {
                                        console.log("line no 2154 isTransModel", this.isTransModel);
                                        this.tablesArray[i][tableKey][j]["table_name"] = cuurentColumneData['table_name'];
                                    }
                                    else 
                                    {
                                        console.log("line no 2159 isTransModel", this.isTransModel);
                                        this.tablesArray[i][tableKey][j]["table_name"] = cuurentColumneData['table_name'];
                                    }
                                    
                                    //Added by Mayur to make changes in Criteria Screen Auto-search on 19/03/21 [Start]

                                    if (cuurentColumneData['table_display_name'] != undefined && cuurentColumneData['table_display_name'] != '') 
                                    {
                                        this.tablesArray[i][tableKey][j]["table_display_name"] = cuurentColumneData['table_display_name'];
                                    }
                                    else {
                                        this.tablesArray[i][tableKey][j]["table_display_name"] = this.tablesArray[i]['DISPLAY_NAME'];
                                    }
                                    ...
                                    if(cuurentColumneData['db_name'] != undefined && cuurentColumneData['db_name'] != '')
                                    {
                                        this.tablesArray[i][tableKey][j]["db_name"] = cuurentColumneData['db_name']; 
                                    }
                                    else
                                    {
                                        this.tablesArray[i][tableKey][j]["db_name"] = ""; 
                                    }
                                   
                                
                                    console.log("line no 2236 this.isTransModel", this.isTransModel);
                                    let val = cuurentColumneData['db_name'];
                                    var column = cuurentColumneData['table_name'] + "." + cuurentColumneData['db_name']
                                        ...
                                        for (const key of Object.keys(this.columnsInGroups)) {
                                            ...
                                            if(value != null)
                                            {
                                                ...
                                                if(value instanceof Array)
                                                {
                                                    ...
                                                    {
                                                        if(this.tablesArray[i][tableKey][j]["db_name"] == value[k])
                                                        {
  ...
  buildSchemaTablesArray()
  {
        ...
        {             
            for( var i=0; i<this.tablesArray.length; i++)
            {
                for (var tableKey of Object.keys(this.tablesArray[i])) 
                {
                    ...
                    {           
                        var isArray = Array.isArray(this.tablesArray[i][tableKey]);
                        if( isArray  && this.tablesArray[i][tableKey].length > 0 )
                        {
                            for( var j=0; j<this.tablesArray[i][tableKey].length; j++)
                            {
                                ...
                                if (curColumneData) 
                                {
                                    ...
                                    if (curColumneData['col_type'] !== undefined) 
                                    {
                                        console.log("line no 2437 this.transModel", this.isTransModel);
                                        if (this.tablesArray[i]['col_type'] && this.tablesArray[i]['col_type'][j]) { 
                                          this.tablesArray[i]['col_type'][j]["type"] = this.getColType(curColumneData['col_type']);
                                        }
                                    } 
                                    else 
                                    {
                                        console.log("line no 2440 this.transModel", this.isTransModel);
    ...
    tablesNewData['ROOT'] = this.tablesArray;
    this.tablesArrForCriteria = JSON.stringify(tablesNewData);
    console.log('print this.tablesArray 2228::::::::',this.tablesArray);
  }
  ...
  onColumnAlignmentSelection(val)
  {
    ...
    onVisualChange(index: any)
    {
       ...
       let tempColumnArr: any = [];   
        for(let i=0; i<allColumns.length; i++)
        {
            ...
            {
                // Added by Nikhil on 21-06-21 on change of visual if the groupbox do not match but the datatype is same then add the column array in the first groupbox of current visuals.
                if((column['StandardName'] != undefined && (item.StandardName == column['StandardName'] && item.AllowedColumnTypes == 'ANY'))  || (column['StandardName'] != undefined && item.StandardName == column['StandardName'] && item.AllowedColumnTypes.includes(column['col_type'])))
                {
                    ind = i;
                    return i;
                }
    ...
    {
        // Changed by Samruddhi for Freehandsql
        if( this.SqlModelData !=undefined && this.SqlModelData['VISUAL_NAME'] != undefined && this.SqlModelData['transaction'] != undefined && this.SqlModelData['transaction']['sql_models'] != undefined && this.SqlModelData['transaction']['sql_models'][0] != undefined && this.SqlModelData['transaction']['sql_models'][0]['sql_model'] != undefined && this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'] != undefined)
        {
            // let sqlModelColumArray = JSON.stringify(this.SqlModelData['SQLModel'][this.param1][0]['COLUMN']);
            let sqlModelColumArray = JSON.stringify(this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns']);
            ...
            for (let j = 0; j < sqlArray.length; j++) 
            {
                var currColumn = sqlArray[j]['column'];
                for (let i = 0; i < this.tablesArray.length; i++) 
                {
                    var columns = this.tablesArray[i]['COLUMN'];
                    

                    if (currColumn['table_name'] === currColumn['table_name']) 
                    {
                        for (let k = 0; k < columns.length; k++) 
                        {
                            var col = columns[k];
                            var dbName = currColumn.db_name;
                            var tblName = currColumn.table_name;

                            if (col.db_name === dbName && col.table_name === tblName) 
                            {
                                let index = columns.findIndex((record: any) => record.db_name === dbName);
                                if (index !== -1) 
                                {
   ...
   onClickGrpBox(index: any)
    ...
    onNextforBrowser() 
    {
        ...
        { 
            ...
            // Changed by Samruddhi for updated UI
            // if(this.currentSchema['baType'] != 'F' && this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'][0]['column'].length <= 0)
           // if(this.listData["baType"] != 'F' && this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'][0]['column'].length <= 0)
            // Changed by Samruddhi for Freehandsql
           if (this.SqlModelData != undefined 
            && this.isSqlView != undefined && this.isSqlView != true) {
        
            let sqlTransaction = this.SqlModelData['transaction'];
            if (sqlTransaction && sqlTransaction['sql_models'] 
                && sqlTransaction['sql_models'][0] 
                && sqlTransaction['sql_models'][0]['sql_model'] 
                && sqlTransaction['sql_models'][0]['sql_model'][0] 
                && sqlTransaction['sql_models'][0]['sql_model'][0]['columns'][0]['column'] != undefined 
                && sqlTransaction['sql_models'][0]['sql_model'][0]['columns'][0]['column'].length <= 0) {
                                    ...
                                    if(finddata != undefined)
                                    {
                                        if(newfinddata != undefined)
                                        {
                                            ...
                                            for(const key of Object.keys(currentJson))
                                            {
                                                ...
                                                else if(key == "col_type" && value == "string")
                                                {
                                                    if(!currentJson.hasOwnProperty('value'))
                                                    {
                                                        currentColunJson['value'] = '';
                                                ...
                                                else if(key == "col_type" && value == "number")
                                                {
                                                    if(!currentJson.hasOwnProperty('value'))
                                                    {
                                                        currentColunJson['value'] = '';
  ...
  generateSQLSeq()
  {
    let groups = this.currentVisual.ColumnGroups;
    // Changed by Samruddhi on 22-04-2022 for visual change issue in freehand
    // this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'][0]['column'] = [];
    if(this.SqlModelData != undefined && this.SqlModelData['SQLModel'] != undefined && this.SqlModelData['SQLModel']['COLUMNS'] != undefined && this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'][0]['column'] != undefined && this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'][0]['column'].length > 0)
    {
        let arrLen = this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'][0]['column'].length;
        this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'][0]['column'].splice(0, arrLen);
        ...
        for(let i = 0; i < groups.length; i++)
        {
            ...
            for(let j = 0; j < columns.length; j++)
            {
                ...
                this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'][0]['column'].push(column);
            }
        }
    }
  }
          ...
          case 'databaseName':{
            ...
            case 'finalSqlModelTblArr':
            {
                // if(!this.isSchemaDesigner)
				{
                    ...
                    {
                        console.log("line no 4186 this.finalTableArray", this.finalTableArray);
  ...
  onLongPress(){
	...
	//Added by shrutika on 12-10-21 for select deselect functionality for schema designer.
        ...
        if (this.SqlModelData != undefined
            && this.SqlModelData['sql_models'] 
            && this.SqlModelData['sql_models'][0] 
            && this.SqlModelData['sql_models'][0]['sql_model'] 
            && this.SqlModelData['sql_models'][0]['sql_model'][0] 
            && this.SqlModelData['sql_models'][0]['sql_model'][0]['columns'][0]['column'] != undefined ) {
            let sqlModelColumArray = this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'][0]['column'];
            for( var i=0;i<sqlModelColumArray.length; i++)
            {
                var currentCol: any = {} = sqlModelColumArray[i];
                if( this.currentDelectedTables.includes(currentCol['table_name']))
                {
    ...
    saveClick()
    {
        console.log("line no 4466 saveClick sql-editor component called::::::", this.SqlModelData)
        var saveData: any = {};
        // Added by Samruddhi to show a alert before saving transactions without columns
        if ( this.SqlModelData != undefined 
            && this.SqlModelData['transaction']['sql_models'] != undefined 
            && this.SqlModelData['transaction']['sql_models'][0]['sql_model'] != undefined
            && this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'].length <= 0)
            {
            console.log("line no 4475 if block")
            window.alert("Folders cannot be empty. Please add Column in the Folder before saving the transaction.");
            return;
        }
        else if (this.SqlModelData != undefined 
            && this.SqlModelData['transaction']['sql_models'] != undefined 
            && this.SqlModelData['transaction']['sql_models'][0]['sql_model'] != undefined
            && this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'].length > 0)
            {
            console.log("line no 4484 :::inside else if")
            saveData['sqlModel'] = this.SqlModelData;
               //Added by shrutika on 05-10-21 for schema designer group box 
            saveData['editorVisuals'] = this.editorVisuals;
            console.log("line no 4519 saveData['editorVisuals']", saveData['editorVisuals'])
            saveData['defaultGrpName'] = this.defaultGrpName;
            console.log("line no 4520 saveData['defaultGrpName']", saveData['defaultGrpName'])
...
//Added by shrutika on 27-09-21 for schema designer [End]
  //Added by vikas for getting current db and connection name in external data source [Start]
    ...
    {
        console.log("line no 4645 oncurrentSelectedDb called::::::::")
    ...
    //   paramMap["databaseName"] = this.currenSelectedTab['name'];
      ...
      else
      {
        ...
        {   
            ...
            var url = this.sqlService.getHostURL() + '/ibase/rest/TransDatabasecatlog/getTables';
        ...
        else
        {
            paramMap["dbDetails"] = ""
            paramMap["databaseName"] = this.currenSelectedTab['name'];
            var url = this.sqlService.getHostURL() + '/ibase/rest/TransDatabasecatlog/tables';
          ...
          //Added by shrutika on 18-10-21 for display error [End]
              ...
              for(var i=0;i< this.oldSlectedTableDataArray.length; i++ )
              {
                  if(firstListTableNames != undefined && !firstListTableNames.toString().includes(this.oldSlectedTableDataArray[i].trim()))
                  {
                      ...
                      console.log("line no 4788 currentTableJson", currentTableJson);
                  }
              }
              this.sqlService.setLoading(false);
...
overLayFoAddGroupBox() 
    ...
    getUserInfo(databaseName: any)
    {
        ...
        var url = this.sqlService.getHostURL() + '/ibase/rest/TransDatabasecatlog/userInfo';
        var paramString = this.sqlService.getEncodedParamString(paramMap);
        this.sqlService.setLoading(true);
        this.sqlService.callRequest(url, paramString).subscribe( (data: any) =>
        {
    ...
    deleteCurrentGroupBox(value:any)
    {
        ...
        for(let i= 0; i < this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'][0]['column'].length; i++)
        {
            if(value == this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'][0]['column'][i]['groupName'] || value == this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'][0]['column'][i]['table_name'])
            {
                this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'][0]['column'].splice(i);
            }
        }
   ...
   //Added by Samruddhi for visual option component [End]
    ...
    openColumnProperties(currentGroupName,columnName,curColName,columnFunct,col_type)
    {
      this.currentGroupName = currentGroupName ;
      this.columnName = columnName ;
      this.currentColName = curColName;
      this.currentColFunct = columnFunct;
      this.coltypecolumnprop = col_type;
    ...
    exportToXmlData()
    {
        console.log("line no 5190 exportToXmlData called::::::", this.SqlModelData);
        console.log("line no 5191 this.SqlModelData['transaction']", this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'][0]['column']);
        console.log("line no 5192 this.SqlModelData['transaction']",this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'][0]['column']);
        // if(this.SqlModelData != undefined)
        if(this.SqlModelData != undefined)
        {
            var columnArray = this.SqlModelData['transaction']['sql_models'][0]['sql_model']['columns'][0]['column'];
		...
		// Changed by Samruddhi for columns not showing after clicking on export button
                console.log("line no 5205 column", column);
                column['checked'] = true;
				column['db_name'] = column['db_name'].toUpperCase();
                console.log("line no 5156 tableNameListArray.includes((column['DBTABLE'])", tableNameListArray.includes((column['DBTABLE'])));
                console.log("line no 5157 tableNameListArray.includes((column['DBTABLE'])", tableNameListArray.includes((column['table_name'])))
            ...
            for( var i=0; i<tableNameListArray.length; i++ )
            {
               ...
               for(var j=0; j<columnArray.length; j++ )
               {
                   ...
                   console.log("line no 5222 currJSON", currJSON);
                   console.log("line no 5223 currJSON['table_name']", currJSON['table_name']);
                   console.log("line no 5224 tableNameListArray[i]", tableNameListArray[i]);
                   console.log("line no 5225 currJSON['DBTABLE']", currJSON['DBTABLE']);
                   if( currJSON['DBTABLE'] == tableNameListArray[i] )
                   {
                       currentTableArray.push(currJSON);
                   }
               }
               console.log("line no 5178 mainDataJSON[tableNameListArray[i]]", mainDataJSON[tableNameListArray[i]]);
            ...
            for(let i=0;i<colArrayName.length;i++)
            {
                ...
                if(tempColJson[colArrayName[i]]['type'] != undefined)
                {
                    ...
                    else if(columnJson['type'] == 'date string' || columnJson['type'] == 'date')
                    {
                        ...
                        columnJson['FUNCTION'] = "";
                        columnJson['EXPRESSIONTYPE'] = "C";
                        columnJson['col_type'] = "DATE";
                        columnJson['PATTERN'] = "dd-MMM-yy"
                    }
                }
                if(tempColJson[colArrayName[i]]['col_type'] != undefined)
                {
                    columnJson['col_type'] = tempColJson[colArrayName[i]]['col_type'];
                    console.log('Print  columnJson[col_type] 5603::::::',columnJson['col_type']);           
                }
                ...
                console.log("line no 5496 this.finalTableArray", this.finalTableArray);
    ...
    setSqlView()
    {
        if(this.listData != undefined && this.listData["baType"] == 'F')
        {
            ...
            if (this.SqlModelData !== undefined 
                ...
                && this.SqlModelData['transaction']['sql_models'][0]['sql_model'] !== undefined 
                && this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0] !== undefined 
                && this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'] !== undefined 
                && this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'][0] !== undefined 
                && this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'][0]['column'] !== undefined) {
                let sqlArray = this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'][0]['column'];
                sqlArray.forEach((column: any) => {
                        column['checked'] = false;
                        return;
                });
                this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'][0]['column'] = sqlArray.filter(item => item.checked);
                // Changed by Samruddhi on 22-04-2022 for visual change issue in freehand
                // this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'][0]['column']= [];
                if(this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'][0]['column'].length > 0)
                {
                    let arrLen = this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'][0]['column'].length;
                    this.SqlModelData['transaction']['sql_models'][0]['sql_model'][0]['columns'][0]['column'].splice(0, arrLen);
                }
        ...
        for(let i=0;i<this.finalColArr.length;i++)
        {
            ...
            if(this.finalColArr[i]['CRITERIA'].includes('?'))
            {
                ...
                for(let j=0;j<newfindData.length;j++)
                {
                    ...
                    {
                        criteriaNewJson['col_type'] = newfindData[j]["type"].toLowerCase();
