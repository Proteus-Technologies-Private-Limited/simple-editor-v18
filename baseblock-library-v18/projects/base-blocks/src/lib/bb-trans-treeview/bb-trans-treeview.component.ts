import { Component, Injectable, OnInit, ViewChild,Output,EventEmitter, ViewEncapsulation, Input, SimpleChanges, ElementRef, TemplateRef, ViewContainerRef } from '@angular/core';
import * as _ from 'lodash';
import { TreeviewI18n, TreeviewItem, TreeviewConfig, TreeviewHelper, TreeviewComponent, TreeviewEventParser, OrderDownlineTreeviewEventParser, DownlineTreeviewItem } from 'ngx-treeview';
import { ApplyTransTemplateDirective } from './apply-trans-template.directive';
import { MatIconRegistry } from '@angular/material/icon';
import { DomSanitizer } from '@angular/platform-browser';
import { SQLTransEditorService } from '../bb-sql-trans-editor/sql-trans-editor-select/sql-trans-editor-select.service';
...
@Component({
  selector: 'bb-trans-treeview',
  templateUrl: './bb-trans-treeview.component.html',
  styleUrls: ['./bb-trans-treeview.component.css'],
...
export class BBTransTreeviewComponent implements OnInit {
    ...
    constructor(private matIconRegistry : MatIconRegistry, private domSanitizer : DomSanitizer, private sqlService: SQLTransEditorService, private overlay: Overlay, private viewContainerRef: ViewContainerRef, private titlecasePipe:TitleCasePipe) {}
    ...
    ngOnInit() {
        ...
        console.log('ngOnInit::isSchemaDesigner: :',this.isSchemaDesigner);
    ...
    private selectItem(selectedItem: TreeviewItem){
        console.log("line no 208 selectItem",selectedItem)
    ...
    setChangeValue( currentColumn: any )
    {
        ...
        console.log('line no 1025 sqlmodelDataY::',currentColumn);
        currentColumn['group_name'] = this.groupName;
        //change by shrutika on 16-07-21 [Start] for display default function on form.
        var cuurentFunction = currentColumn['default_function'];
        currentColumn['default_function'] = cuurentFunction;
        ...
        if(this.isSchemaDesigner == true)
        {
            ...
            currentColumn['heading'] =this.titlecasePipe.transform(currentColumn['heading']);
        ...
        if(this.editFlag == 'E')
        {
            if(this.tablesArray !=undefined )
            {
                for(let j = 0; j< this.tablesArray.length; j++)
                {
                    ...
                    {
                        console.log("line no 282 this.currentVisual['ColumnGroups']", this.currentVisual['ColumnGroups']);
    ...
    setColumns(columnInRowsGrp)
    {
        ...
        try
        {
            console.log("line no 455 this.tablesArray", this.tablesArray)
                    ...
                    {
                        {
                           for(let j = 0; j < this.tablesArray[ind]['COLUMN'].length; j++)
                           {
                                ...
                                console.log("line no 466 currentColJson", currentColJson);
    ...
    onRightClick(event: any,tableNameNew:any)
    {
        ...
        console.log("line ni 496 this.tablesArray", this.tablesArray);
        setTimeout(() => {
    ...
    onclickpopup(event: any)
    {
        ...
        console.log("line no 570 this.tablesArray", this.tablesArray)
        setTimeout(() => {
            for(let i = 0; i< this.tablesArray.length; i++)
            {
                if(this.tablesArray[i]['TABLE_NAME'] ==  this.currentTable.toUpperCase())
