...
export class ExtractTemplateService 
{
    ...
    sendRequest(url:any, paramString:any, cllback:any,validationKey?:any)
    {
        ...
        {
            console.error('Exception inside sendRequest:: ', e.message);
        }
    }
    ...
    checkErrorException(response:any, callback:any,validationKey?:any)
    {
        try
        {
            ...
            {
                var errorDom = parser.parseFromString(response, "text/xml");
                var errorType:any;
                var errorId:any;
                var msg:any = "";
                var descr:any = "";
                ...
                // Cache getElementsByTagName results to avoid repeated DOM queries
                var errorElements = errorDom.getElementsByTagName("error");
                var messageElements = errorDom.getElementsByTagName("message");
                var descriptionElements = errorDom.getElementsByTagName("description");
                var traceElements = errorDom.getElementsByTagName("trace");
                var errorLen:any = errorElements.length;
                for (var i = 0; i < errorLen; i++)
                {
                    if (errorElements[i] != null)
                    {
                        errorColName = errorElements[i].getAttribute("column_name");
                        errorType = errorElements[i].getAttribute("type");
                        errorId = errorElements[i].getAttribute("id");
                    }

                    if (messageElements[i] != null && messageElements[i].childNodes[0] != null)
                    {
                        msg = messageElements[i].childNodes[0].nodeValue;
                    }

                    if (descriptionElements[i] != null && descriptionElements[i].childNodes[0] != null)
                    {
                        descr = descriptionElements[i].childNodes[0].nodeValue;
                    }

                    if (traceElements[i] != null && traceElements[i].childNodes[0] != null)
                    {
                        trace = traceElements[i].childNodes[0].nodeValue;
                    }
        ...
        {
            console.error('Exception inside checkErrorException method ', e.message);
        }
    }
